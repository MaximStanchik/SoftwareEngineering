...
int main() {
	...
	int data, day, month, year,A, days{}, Birthday, YearOfBirthday, NumberOfcurrentDay, currentday, currentmonth, currentyear, left;
	int count = 0;
	...
	currentday = DATA / 1000000; //потому что переменная содержит восемь цифр и шесть цифр у нас отсекается у даты. получаем просто день. 
	currentmonth = (DATA % 1000000) / 10000;
	currentyear = DATA % 10000;
	...
	if (currentyear == YearOfBirthday && Birthday>NumberOfcurrentDay) {
	...
	if (YearOfBirthday==currentyear+1 && currentyear % 4 == 0) {
	...
	if (YearOfBirthday > currentyear + 1) {
	...
	if (currentyear == YearOfBirthday && NumberOfcurrentDay > NumberOfcurrentDay && currentyear % 4 == 0) {
		left =366- NumberOfcurrentDay+ Birthday;
		cout << "До вашего дня рождения осталось " << left << " дней"<< endl;
		cout << "Программа закончила свое выполнение. Благодарим за использование нашего приложения" << endl;
		return 0;
	...
	if (month >currentmonth && YearOfBirthday>currentyear) {
